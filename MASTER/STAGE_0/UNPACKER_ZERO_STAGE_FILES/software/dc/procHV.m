function [errorOnClose, newFilename] = procHV(fp,fileName,outPath,systemName,dev2Read,logs,alarms,interpreter,OS)
...
try
    while ~feof(fp)
        ...
        [result1, lastPosition, time_] = checkTimeFormat(string,'I2C_4','');
        if result1 %time check format passed
            [result2, lastPosition, ID_] = checkIDFormat(string,lastPosition);
            if result2
                [result3, lastPosition, data_] = checkColFormat(string,lastPosition,16);
                if result3
                    data = [data;  data_];
                    time = [time;  time_];
                end
