...
Pulls lab log files from a station, cleans them, and updates STAGE_1 status.

Usage:
  log_bring_and_clean.sh <station>

Options:
  -h, --help    Show this help message and exit.

Pass the station number (1-4). The script fetches raw logs via rsync, runs the
cleaner, and updates metadata tracking for downstream aggregation.
EOF
  exit 0
fi
...
python_script_path="$HOME/DATAFLOW_v3/MASTER/STAGE_1/LAB_LOGS/log_aggregate_and_join.py"

base_working_directory="$HOME/DATAFLOW_v3/STATIONS/MINGO0${station}/STAGE_1/LAB_LOGS"
