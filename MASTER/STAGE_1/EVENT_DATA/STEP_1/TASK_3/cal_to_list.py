...
from common.pipeline_core import StageMetadata, stage3_cal_to_list  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="STEP 1 · TASK 3 — convert calibrated data into LIST-level artifacts."
    )
    ...
    parser.add_argument(
        "--source",
        type=Path,
        help="Optional override to process a specific calibrated artifact.",
    )
...
def main() -> None:
    ...
    task_root = station_paths.task_dirs["TASK_3"]
    work_dirs = ensure_task_layout(task_root)
    status_csv = (task_root / "LOGS" / "cal_to_list_status.csv")
    status_timestamp = append_status_row(status_csv)

    stage4_dirs = ensure_task_layout(station_paths.task_dirs["TASK_4"])

    upstream_dirs = ensure_task_layout(station_paths.task_dirs["TASK_2"])
    ...
    if args.source:
        ...
        with acquire_file(
            ...
            if candidate_path is None:
                print("[cal_to_list] No calibrated artifacts to process.")
    ...
    print(f"[cal_to_list] Processing {processing_path.name}")
    ...
    try:
        ...
        listed_df = stage3_cal_to_list(df, config, stage_metadata)

        output_name = processing_path.stem.replace("calibrated_", "listed_")
        output_path = stage4_dirs["unprocessed"] / f"{output_name}.h5"
        save_dataframe_h5(listed_df, output_path)

        metadata.update(
            {
                "stage": "cal_to_list",
                "listed_rows": int(len(listed_df)),
        ...
        mark_status_complete(status_csv, status_timestamp)
        print(f"[cal_to_list] Produced {output_path.name}")
    except Exception as exc:
        fail_file(processing_path, work_dirs["error"])
        print(f"[cal_to_list] Failed to process {processing_path.name}: {exc}")
        raise
    finally:
