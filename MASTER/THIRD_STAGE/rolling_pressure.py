...
# %%
# ==============================================================================
# NEW CODE ADDED FOR CORRECTION COMPARISON (NaN-safe version)
# ==============================================================================

# Clean up previous columns if they exist
cols_to_drop = ['day', 'BEST_BETA', 'NEW_CORRECTED']
...
# Forward-fill and back-fill missing beta values
merged_df['BEST_BETA'] = merged_df['BEST_BETA'].fillna(method='ffill').fillna(method='bfill')

# If there are still NaN values, use the median of available beta values
if merged_df['BEST_BETA'].isnull().any():
    median_beta = best_beta_df['BEST_BETA'].median()
    merged_df['BEST_BETA'].fillna(median_beta, inplace=True)
    print(f"Warning: Filled remaining NaN BEST_BETA values with median: {median_beta:.6f}")


plt.figure(figsize=(10, 6))
plt.plot(best_beta_df['day'], best_beta_df['BEST_BETA'], lw=2)
plt.title("Best beta values")
plt.xlabel("Date")
plt.ylabel("Beta Value")
plt.grid(True)
plt.legend()
...
# 3. Create a new column 'NEW_CORRECTED' with NaN handling
# Calculate the overall mean pressure (excluding NaN values)
pressure_mean_overall = merged_df['RPRESS'].mean()

# Create boolean mask for valid data (no NaN in required columns)
valid_mask = (
    merged_df['RUNCORR'].notna() & 
    merged_df['BEST_BETA'].notna() & 
    merged_df['RPRESS'].notna()
)

# Initialize NEW_CORRECTED column with NaN
merged_df['NEW_CORRECTED'] = np.nan

# Apply correction formula only to valid data
# Corrected Rate = Uncorrected_Rate / exp(beta * (Pressure - Mean_Pressure))
merged_df.loc[valid_mask, 'NEW_CORRECTED'] = (
    merged_df.loc[valid_mask, 'RUNCORR'] / 
    np.exp(merged_df.loc[valid_mask, 'BEST_BETA'] * 
           (merged_df.loc[valid_mask, 'RPRESS'] - pressure_mean_overall))
)

print(f"Created 'BEST_BETA' and 'NEW_CORRECTED' columns in merged_df.")
print(f"Valid data points: {valid_mask.sum()} out of {len(merged_df)}")

# 4. Plot the time series of 1HCOR_E vs RPRESS and NEW_CORRECTED vs RPRESS
# Create masks for plotting (exclude NaN values)
plot_mask_old = merged_df['1HCOR_E'].notna() & merged_df['RPRESS'].notna()
plot_mask_new = merged_df['NEW_CORRECTED'].notna() & merged_df['RPRESS'].notna()

fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(15, 10), sharex=True, sharey=True)

# Plot for the original correction (only valid data)
if plot_mask_old.any():
    ax1.scatter(merged_df.loc[plot_mask_old, 'RPRESS'], 
                merged_df.loc[plot_mask_old, '1HCOR_E'], 
                alpha=0.8, s=2, label='1HCOR_E (Old Correction)')
ax1.set_ylabel('Corrected Rate')
ax1.set_title('Old Correction vs. Pressure')
ax1.grid(True)
ax1.legend()

# Plot for the new correction (only valid data)
if plot_mask_new.any():
    ax2.scatter(merged_df.loc[plot_mask_new, 'RPRESS'], 
                merged_df.loc[plot_mask_new, 'NEW_CORRECTED'], 
                alpha=0.8, s=2, label='NEW_CORRECTED (New Correction)', color='g')
...
# 5. Calculate and print the correlation coefficients (NaN-safe)
# Create masks for correlation calculation
corr_mask_old = merged_df['1HCOR_E'].notna() & merged_df['RPRESS'].notna()
corr_mask_new = merged_df['NEW_CORRECTED'].notna() & merged_df['RPRESS'].notna()

print("\n--- Correlation Analysis ---")

if corr_mask_old.sum() > 1:
    corr_old = merged_df.loc[corr_mask_old, '1HCOR_E'].corr(merged_df.loc[corr_mask_old, 'RPRESS'])
    print(f"Correlation between '1HCOR_E' (Old) and 'RPRESS': {corr_old:.6f} (n={corr_mask_old.sum()})")
else:
    corr_old = np.nan
    print("Cannot calculate correlation for '1HCOR_E' - insufficient valid data")

if corr_mask_new.sum() > 1:
    corr_new = merged_df.loc[corr_mask_new, 'NEW_CORRECTED'].corr(merged_df.loc[corr_mask_new, 'RPRESS'])
    print(f"Correlation between 'NEW_CORRECTED' and 'RPRESS': {corr_new:.6f} (n={corr_mask_new.sum()})")
else:
    corr_new = np.nan
    print("Cannot calculate correlation for 'NEW_CORRECTED' - insufficient valid data")

# Compare correlations only if both are valid
if not (np.isnan(corr_old) or np.isnan(corr_new)):
    ...
    print("\nCannot compare correction methods due to insufficient valid data.")

# Print data quality summary
print(f"\n--- Data Quality Summary ---")
print(f"Total rows: {len(merged_df)}")
print(f"Valid RPRESS values: {merged_df['RPRESS'].notna().sum()}")
print(f"Valid RUNCORR values: {merged_df['RUNCORR'].notna().sum()}")
print(f"Valid 1HCOR_E values: {merged_df['1HCOR_E'].notna().sum()}")
print(f"Valid NEW_CORRECTED values: {merged_df['NEW_CORRECTED'].notna().sum()}")
print(f"Valid BEST_BETA values: {merged_df['BEST_BETA'].notna().sum()}")
